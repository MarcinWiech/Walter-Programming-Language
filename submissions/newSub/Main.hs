...
-- given a function name and an environment returns the environment of that function
...
-- initialises global environment                                                 
...
-- updates function's environment
...
-- returns Int value of a variable
getMValueInt :: M -> Int
getMValueInt (MInt _ v) = v                                            

-- returns Bool value of a variable
getMValueBool :: M -> Bool
getMValueBool (MBool _ v) = v   

-- given function name, environment, variable name returns if the given variable exists in function's environment
...
-- given a function name, environment and variable name returns that variable in the "M" form
...
-- given function name, environment and variable name returns integer value of that variable
...
-- given function name, environment and variable name returns bool value of that variable
...
-- given function name, environment and variable "M" adds or updates this variable in the function's environment
...
-- executes the main function, fails if the main function isn't the head of the list
eval1_findMain :: [FuncDeclaration_] -> IO ()
...
eval1_findMain ((MainFuncDeclaration (MultipleSegue funcname next)):ss) = evalFunction funcname (envInit ss) ss match
                                                                          where (NormalFuncDeclaration _ initArea match) = findFunctionByName funcname ss

-- given function name, and all the functions returns this function declaration
...
-- evaluates match and expression of the function's body
...
-- unfolds a match and returns the expression linked to it
...
-- given a function name, environment, list of variables and list of their values updates the environment for that function
...
-- reads a line from stdio and returns the list of integers
matchIntFromStdio :: IO [Int]
matchIntFromStdio =  do line <- getLine
                        return $ (map read $ words line :: [Int])

-- convert comparable expressions to an integer values (True = 1, False = 0)
...
-- evaluates a single output pattern to an integer value (True = 1, False = 0)
...
-- evaluates maths expressions to MathsInt
...
-- print output pattern to stdout
...
-- evaluates single comparable expression
