...
happyExpList = Happy_Data_Array.listArray (0,53) ([8192,0,32768,0,57344,1,0,8,0,30,0,128,0,512,0,2048,0,8192,0,30720,1,0,0,0,0,0,0,0,96,0,384,0,0
	])

{-# NOINLINE happyExpListPerState #-}
happyExpListPerState st =
    token_strs_expected
  where token_strs = ["error","%dummy","%start_parseCalc","Exp","Type","intValue","var","funcName","'='","'+'","'-'","'*'","'/'","'('","')'","':'","'['","']'","'{'","'}'","','","'>>'","intType","eof","boolType","';'","if","else","main","trueValue","falseValue","'=='","'!'","%eof"]
        bit_start = st * 34
        bit_end = (st + 1) * 34
...
happyNewToken action sts stk (tk:tks) =
	...
	case tk of {
	TokenIntValue _ happy_dollar_dollar -> cont 6;
	TokenVar _ happy_dollar_dollar -> cont 7;
	TokenFuncName _ happy_dollar_dollar -> cont 8;
	TokenEq _ -> cont 9;
	TokenPlus _ -> cont 10;
	...
	TokenLBracket _ -> cont 17;
	TokenRBracket _ -> cont 18;
	TokenLCurlyBracket _ -> cont 19;
	TokenRCurlyBracket _ -> cont 20;
	TokenComma _ -> cont 21;
	TokenPipe _ -> cont 22;
	TokenTypeInt _ -> cont 23;
	TokenEOF _ -> cont 24;
	TokenTypeBool _ -> cont 25;
	TokenSemiColon _ -> cont 26;
	TokenIf _ -> cont 27;
	TokenElse _ -> cont 28;
	TokenMain _ -> cont 29;
	TokenTrueValue _ -> cont 30;
	TokenFalseValue _ -> cont 31;
	TokenCompare _ -> cont 32;
	TokenNot _ -> cont 33;
	_ -> happyError' ((tk:tks), [])
	}
